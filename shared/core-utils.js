...
function _findTimeText(base){
  ...
  /* 欄名關鍵字都對不上時的最後防線：直接看值本身格式，
     命中「YYYY/M/D」或「YYYY-MM-DD」開頭、或帶「上午/下午」＋時:分 的字串 */
  var dateLike = /^\d{4}[\/\-]\d{1,2}[\/\-]\d{1,2}|(上午|下午).{0,3}\d{1,2}:\d{2}/;
  for (var n=0;n<keys.length;n++){
    var v3 = base[keys[n]];
    var s3 = (v3===undefined||v3===null) ? '' : String(v3).trim();
    if(s3 && dateLike.test(s3)) return s3;
  }
  return '';
}
